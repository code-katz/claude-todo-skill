...
## TODOS.md vs Claude Code's Native Task List

These are different tools for different lifetimes. Claude Code's built-in task tools (TaskCreate, TaskUpdate, TaskList) track THIS session's execution steps; that list is machine-local and does not survive as project state. TODOS.md is the opposite: a committed, cross-session backlog with persona routing. Rule of thumb: steps you are executing right now belong in the native task list; ideas and tasks that must still exist next week belong in TODOS.md. Do not mirror items between the two.
